...
# intersect(substr(fs,3,nchar(fs)-5),tnames)
# setdiff(substr(fs,3,nchar(fs)-5),tnames)
# setdiff(tnames,substr(fs,3,nchar(fs)-5))
...
dimnames(LMcoords) <- list(chars,c("X","Y","Z"),c(shared,"Cb_9St","Cb_9Bo","Hn_9St","Hn_9Bo"))
...
## Adding Ichthyosauromorpha
tnew$Clade1[50:53] <- c("Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Clade2[50:53] <- c("Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Family[50:53] <- c("Chaohusauridae","Chaohusauridae","Hupehsuchidae","Hupehsuchidae")
tnew$Genus[50:53] <- c("Chaohusaurus","Chaohusaurus","Hupehsuchus","Hupehsuchus")
tnew$Species[50:53] <- c("St","Bo","St","Bo")
...
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_9_3D_shared.pdf")
...
pdf("Cetacea_9_PCA_TPS_3D_shared.pdf",h=8.5,w=11)
  layout(matrix(seq(1,4),2,2))
  plotRefToTarget(meanLM,pca.res$shapes$shapes.comp1$min)
...
pdf('Bt_9_3D_shared.pdf', width=9, height=6.5)
...
pdf(h=4.5, w=4, file="AllLM_9_shared.pdf")
  plotAllSpecimens(shape.data)
  for(i in 1:9) add.ellipse.one(shape.data[i,1,],shape.data[i,2,],series=i)
dev.off()

plotLD(shape.data,file="LM_9_shared.pdf")
...
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_9_2D_shared.pdf")
layout(matrix(seq(1,4),2,2))
  chplot(pca.coord[,1],pca.coord[,2],tnew$Clade2,plotlegend=F)
  chplot(pca.coord[,1],pca.coord[,2],tnew$Clade2,plotlegend=T)
  chplot(pca.coord[,1],pca.coord[,2],tnew$Family,plotlegend=F)
  chplot(pca.coord[,1],pca.coord[,2],tnew$Family,plotlegend=T)
dev.off()
...
# Open PDF graphics device
pdf('Bt_9_2D_shared.pdf', width=9, height=6.5)
