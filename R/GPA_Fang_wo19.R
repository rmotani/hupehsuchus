...
DAT <- DAT[DAT$Clade1 != "Ichthyosauromorpha",]
...
rmv <- c("Balaenoptera_borealis",
         ...
         "Feresa_attenuata",
         "Delphinus_delphis")
for(i in 1:length(rmv)) DAT <- DAT[rownames(DAT) != rmv[i],]
fnames <- paste0(DAT$Genus,"_",DAT$Species)
#rownames(DAT) <- fnames <- paste0(DAT$Genus,"_",DAT$Species)
...
fnames2 <- paste0(substr(DAT$Genus,1,5),"_",substr(DAT$Species,1,5))
dimnames(LMcoords) <- list(chars,c("X","Y"),c(fnames2,"Cb_9St","Cb_9Bo","Hn_9St","Hn_9Bo"))

fam <- c(DAT$Family,"Cb_9St","Cb_9Bo","Hn_9St","Hn_9Bo")
cl2 <- c(DAT$Clade2,"Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha")
...
pdf(h=4.5, w=4, file="AllLM_Fang_wo19.pdf")
...
summary(pca.res)
pca.coord <- pca.res$x
pointcolor <- rep(c("steelblue4","plum4","royalblue4","gold4","salmon4","seagreen4","turquoise4","mistyrose4","lightgoldenrod4","palevioletred4"),3)
fillcolor <- scales::alpha(pointcolor,0.3) 
pointsymbol <- rep(c(4,3,15,8,17,19,0,1,2,5,6,7),3)
pdf(h=9,w=8,file="PCA_Fang_wo19.pdf")
...
pdf(h=11,w=11,file="PCA_Fang_wo19_wNames.pdf")
  chplot(pca.coord[,1],pca.coord[,2],cl2,plotlegend=F,xlab="PC 1",ylab="PC2")
  textxy(pca.coord[,1],pca.coord[,2],fnames2)
dev.off()

plotLD(shape.data,file="LM_Fang_wo19.pdf")
...
pdf('Bt_Fang_wo19.pdf', width=9, height=6.5)
