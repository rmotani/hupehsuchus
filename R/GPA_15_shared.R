...
fnames <- paste0(CET$Genus,"_",CET$Species)
...
fs <- list.files("../3D_Cetacea_Principal/@fcsv",pattern=".fcsv")
chars <- paste0("F-",seq(1,15))

LMcoords <- NULL
for(i in 1:length(fs)){
  tmpdat <- read.csv(paste("../3D_Cetacea_Principal/@fcsv/",fs[i],sep=""),skip=3,header=F)
  rownames(tmpdat) <- tmpdat$V12
  tmpdat2 <- tmpdat[chars,2:4]
  LMcoords <- abind(LMcoords,tmpdat2,along=3)
}
dimnames(LMcoords) <- list(chars,c("X","Y","Z"),substr(fs,1,nchar(fs)-5))
intersect(substr(fs,1,nchar(fs)-5),tnames)
setdiff(substr(fs,1,nchar(fs)-5),tnames)
setdiff(tnames,substr(fs,1,nchar(fs)-5))
LMcoords <- LMcoords[,,shared]

## Adding Ichthyosauromorpha
Cb <- read.csv("Cb_15.csv",row.names=1)
LMcoords <- abind(LMcoords,Cb,along=3)
Hn <- read.csv("Hn_15.csv",row.names=1)
LMcoords <- abind(LMcoords,Hn,along=3)
dimnames(LMcoords) <- list(chars,c("X","Y","Z"),c(shared,"Chaohusaurus","Hupehsuchus"))
...
nlevels(as.factor(tnew$Family))
nlevels(as.factor(tnew$Genus))

## Adding Ichthyosauromorpha
tnew$Clade1[50:51] <- c("Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Clade2[50:51] <- c("Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Family[50:51] <- c("Chaohusauridae","Hupehsuchidae")
tnew$Genus[50:51] <- c("Chaohusaurus","Hupehsuchus")
tnew$Species[50:51] <- c("brevifemoralis","nanchangensis")
...
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_15_3D_shared.pdf")
...
pdf("Cetacea_15_PCA_TPS_3D_shared.pdf",h=8.5,w=11)
  layout(matrix(seq(1,4),2,2))
  plotRefToTarget(meanLM,pca.res$shapes$shapes.comp1$min)
...
# plot3d(MeshIn,type="shade",col="lightgreen",aspect=F)
# MeshOutMean_15 <- warpRefMesh(MeshIn,LMcoords[,,"Orcinus_orca"],meanLM)
# mesh2obj(MeshOutMean_15,"Orcinus_orca_meanLM_15.obj")
# 
# MeshOutPC1min_15 <- plotRefToTarget(meanLM,pca.res$shapes$shapes.comp1$min,method="surface",mesh=MeshOut_15,mesh.coord=meanLM)
# mesh2obj(MeshOutPC1min_15,filename="Orcinus_orca_C1min_15.obj")
# MeshOutPC1max_15 <- plotRefToTarget(meanLM,pca.res$shapes$shapes.comp1$max,method="surface",mesh=MeshOut_15,mesh.coord=meanLM)
# mesh2obj(MeshOutPC1max_15,filename="Orcinus_orca_C1max_15.obj")
# MeshOutPC2min_15 <- plotRefToTarget(meanLM,pca.res$shapes$shapes.comp2$min,method="surface",mesh=MeshOut_15,mesh.coord=meanLM)
# mesh2obj(MeshOutPC2min_15,filename="Orcinus_orca_C2min_15.obj")
# MeshOutPC2max_15 <- plotRefToTarget(meanLM,pca.res$shapes$shapes.comp2$max,method="surface",mesh=MeshOut_15,mesh.coord=meanLM)
# mesh2obj(MeshOutPC2max_15,filename="Orcinus_orca_C2max_15.obj")
...
pdf('Bt_15_3D_shared.pdf', width=9, height=6.5)
...
pdf(h=4.5, w=4, file="AllLM_15_shared.pdf")
  plotAllSpecimens(shape.data)
  for(i in 1:15) add.ellipse.one(shape.data[i,1,],shape.data[i,2,],series=i)
dev.off()

plotLD(shape.data,file="LM_15_shared.pdf")
...
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_15_2D_shared.pdf")
layout(matrix(seq(1,4),2,2))
  chplot(pca.coord[,1],-pca.coord[,2],tnew$Clade2,plotlegend=F,xlab="PC 1",ylab="-PC2")
  chplot(pca.coord[,1],-pca.coord[,2],tnew$Clade2,plotlegend=T,xlab="PC 1",ylab="-PC2")
  chplot(pca.coord[,1],-pca.coord[,2],tnew$Family,plotlegend=F,xlab="PC 1",ylab="-PC2")
...
# Open PDF graphics device
pdf('Bt_15_2D_shared.pdf', width=9, height=6.5)
