...
fnames2 <- paste0(substr(CET$Genus,1,5),"_",substr(CET$Species,1,5))
...
plotLD(shape.data,file="LM_Fang_cetacea_asis.pdf")

meanLM <- mshape(shape.data)
pca.res <- gm.prcomp(shape.data)
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_Fang_cetacea.pdf")
...
pdf(h=11,w=11,file="PCA_Fang_Cetacea_wNames.pdf")
  chplot(pca.coord[,1],pca.coord[,2],cl2,plotlegend=F,xlab="PC 1",ylab="PC2")
...
pdf('Bt_Fang_cetacea.pdf', width=9, height=6.5)
