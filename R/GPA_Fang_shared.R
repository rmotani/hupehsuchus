...
###
### Reading taxonomy from 3D data set
###

taxa <- read.csv("Cetacea.csv")
taxa <- taxa[,-ncol(taxa)]
tnames <- rownames(taxa) <- paste0(taxa$Genus,"_",taxa$Species)
...
shared <- intersect(fnames,tnames)
setdiff(fnames,tnames)
setdiff(tnames,fnames)
...
dimnames(LMcoords) <- list(chars,c("X","Y"),c(shared,"Cb_9St","Cb_9Bo","Hn_9St","Hn_9Bo"))

fam <- c(CET[shared,]$Family,"Cb_9St","Cb_9Bo","Hn_9St","Hn_9Bo")
cl2 <- c(CET[shared,]$Clade2,"Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha","Ichthyosauromorpha")
...
pdf('Bt_Fang_shared.pdf', width=9, height=6.5)
