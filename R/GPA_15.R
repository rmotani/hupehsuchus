...
dimnames(LMcoords) <- list(chars,c("X","Y","Z"),c(substr(fs,1,nchar(fs)-5),"Chaohusaurus","Hupehsuchus"))
...
## Adding Ichthyosauromorpha
tnew$Clade1[66:67] <- c("Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Clade2[66:67] <- c("Ichthyosauromorpha","Ichthyosauromorpha")
tnew$Family[66:67] <- c("Chaohusauridae","Hupehsuchidae")
tnew$Genus[66:67] <- c("Chaohusaurus","Hupehsuchus")
tnew$Species[66:67] <- c("brevifemoralis","nanchangensis")
cbind(dimnames(LMcoords)[[3]],rownames(tnew),sort(rownames(taxa)))
dimnames(LMcoords) <- list(chars,c("X","Y","Z"),)
...
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_15_3D.pdf")
...
pdf("Cetacea_15_PCA_TPS_3D.pdf",h=8.5,w=11)
  layout(matrix(seq(1,4),2,2))
  plotRefToTarget(meanLM,pca.res$shapes$shapes.comp1$min)
...
pdf('Bt_15_3D.pdf', width=9, height=6.5)
...
pdf(h=4.5, w=4, file="AllLM_15.pdf")
  plotAllSpecimens(shape.data)
  for(i in 1:15) add.ellipse.one(shape.data[i,1,],shape.data[i,2,],series=i)
dev.off()

plotLD(shape.data,file="LM_15.pdf")

meanLM <- mshape(shape.data)
pca.res <- gm.prcomp(shape.data)
summary(pca.res)
pca.coord <- pca.res$x
pdf(h=9,w=8,file="PCA_15_2D.pdf")
layout(matrix(seq(1,4),2,2))
  chplot(pca.coord[,1],pca.coord[,2],tnew$Clade2,plotlegend=F,xlab="PC 1",ylab="PC2")
  chplot(pca.coord[,1],pca.coord[,2],tnew$Clade2,plotlegend=T,xlab="PC 1",ylab="PC2")
  chplot(pca.coord[,1],pca.coord[,2],tnew$Family,plotlegend=F,xlab="PC 1",ylab="PC2")
...
# Open PDF graphics device
pdf('Bt_15_2D.pdf', width=9, height=6.5)
